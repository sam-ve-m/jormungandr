    ...
    with patch.object(Config, "__init__", return_value=None):
        with patch.object(Config, "__call__"):
            with patch.object(logging.config, "dictConfig"):
                ...
                from func.main import get_onboarding_step_br
...
@mark.asyncio
@patch.object(OnboardingSteps, "onboarding_user_current_step_br")
@patch.object(Heimdall, "decode_payload")
async def test_get_onboarding_step_br_when_request_is_ok(
    ...
    with app.test_request_context(
        ...
        get_onboarding_step_br_result = await get_onboarding_step_br(request)

        assert (
            get_onboarding_step_br_result.data
...
@patch.object(OnboardingSteps, "onboarding_user_current_step_br")
@patch.object(Heimdall, "decode_payload")
async def test_get_onboarding_step_br_when_jwt_is_invalid(
...
@patch.object(OnboardingSteps, "onboarding_user_current_step_br")
@patch.object(Heimdall, "decode_payload")
async def test_get_onboarding_step_br_when_generic_exception_happens(
    decode_payload_mock, onboarding_steps_mock, etria_mock
):
    decode_payload_mock.return_value = (decoded_jwt_ok, HeimdallStatusResponses.SUCCESS)
